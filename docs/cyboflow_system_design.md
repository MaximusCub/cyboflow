...
> **Historical — MVP-era product spec.** This document is the original two-week-MVP plan and
> has drifted from the shipped app in ways beyond what the inline correction callouts below
> capture. `docs/ARCHITECTURE.md` is current truth for architecture, data model, and the flow
> roster.
...
> **Workflow customization has since shipped (current source of truth).** Choice 1 above was the original MVP scope cut, not a permanent limit. The app now has an in-app step editor (`WorkflowEditorCanvas`), per-agent model/runtime configs (`WorkflowStepInspector`, the `agentConfigs` overlay on a workflow definition), and A/B workflow variants (`cyboflow_create_variant` / `cyboflow_list_variants`). See `docs/ARCHITECTURE.md` for the current surface.
...
**The thing being built.** Cyboflow is a **self-contained** desktop app that orchestrates Claude Code as a multi-agent workflow runner. It ships **five user-facing native flows** — **Launch** (interview a brand-new project into a brief, an ordered idea set, and a first build set), **Planner** (turn a raw idea into a reviewed backlog of epics + tasks), **Sprint** (execute the ready tasks), **Compound** (mine merged runs for durable learnings, launched from the Insights view), and **Ship** (take a raw idea end to end — approve the stub, expand the spec, decompose it, materialize a sprint, and drive every approved task to integration in one continuous run) — whose prompt bodies live in the app source; there is no runtime dependency on any external workflow-runner plugin. Users start a flow against a repo; the app spawns Claude Code in an isolated git worktree per run, parses the structured stream-json output, surfaces the work in a custom UI, writes the app's own DB-canonical backlog, and concentrates everything that needs human attention into a single review queue.
...
> **Naming note (P0 SoloFlow rip-out).** This document predates the rebuild that brought the flows in-app. Historical references below to "five pre-set SoloFlow workflows" describe the original fork posture; the shipped app exposes five user-facing built-in flows (Launch, Planner, Sprint, Compound, Ship — `compound` was rebuilt natively from the preserved prose and `ship` was built natively from scratch; `launch` shipped later still). A sixth, internal `verify-setup` flow is registered but picker-hidden. The dropped `prune` flow's prose is preserved under `docs/workflows-future/` for a future cyboflow-native rebuild (`compound`'s preserved prose also remains there, though `compound` has since shipped natively); `soloflow` was the external plugin this app's flows originated from, not a dropped flow. The `__quick__` sentinel remains an internal, picker-hidden lightweight path.
...
> **Codex cut reversed (current source of truth).** The v1 "no Codex/OpenAI integration" call above did not hold: Codex now ships as a second agent provider alongside Claude Code (`CodexPtyManager` / `CodexSdkManager` under `main/src/services/panels/codex/`), selectable per-agent via a workflow's `agentConfigs` overlay. See `docs/ARCHITECTURE.md` for the current dual-provider surface.
...
- `AbstractCliManager` and `CliManagerFactory` for PTY-based Claude Code session management. This is an intentional extension surface: keep `AbstractCliManager` abstract with per-tool concrete managers (the Codex managers now extend it alongside the Claude ones) — do not collapse it into a single concrete class.
...
- **Codex/OpenAI integration — delete.** Crystal supports both Claude Code and OpenAI Codex as agent backends, with separate manager classes and UI surfaces. Cyboflow's thesis is built on Claude Code's stream-json output and Claude's permission-prompt-tool mechanism; the Codex paths are dead weight that complicate the codebase for no v1 benefit. Multi-provider support, if ever wanted, should be designed deliberately rather than inherited.
  ...
  > **Reversed (current source of truth).** This cut did not stick: Codex now ships as a second agent provider, with its own `CodexPtyManager` / `CodexSdkManager` (`main/src/services/panels/codex/`) alongside the Claude managers, both extending the same `AbstractCliManager` extension surface (see the extension-surface note above) — routed per-agent via workflow `agentConfigs`.
...
> **Entity model + review queue (post-MVP rebuild — current source of truth).** The "five new tables" framing below is the original run-substrate design and is still accurate for the *run* tables (`workflows`, `workflow_runs`, `raw_events`, `messages`, `approvals`). On top of it, the app now persists a **DB-canonical 3-table entity model** — `ideas` / `epics` / `tasks` (migration 015), each with its own columns + a single markdown `body`, sharing **one 5-stage board** (migration 042 collapsed the original 12-stage board to 4 kept stages and migration 066 re-added the derived position-7 "In development" stage; union view across all three types, decomposed ideas go off-board via `ideas.decomposed_at` rather than a board stage). A polymorphic `entity_events` audit log replaces the task-scoped `task_events`. All entity writes funnel through the single `TaskChangeRouter.applyChange` chokepoint. A unified **`review_items`** inbox (migration 016) backs the review queue: `kind in (finding|permission|decision|human_task|notification)` (the fifth kind added by migration 046), per-item `blocking`, a soft polymorphic `(entity_type, entity_id)` entity link. Normal review-item writes go through the async `ReviewItemRouter`; the one sanctioned exception is a synchronous folded-gate co-write in `main/src/orchestrator/reviewItemListing.ts`, used by the run-pause routers (approvals, questions, human gates) so an approval/question/human-gate row commits atomically with its `review_items` row in the same transaction. The flow agents — every entry in `CYBOFLOW_WORKFLOW_NAMES` (type `CyboflowWorkflowName`, guard `isCyboflowWorkflowName` in `shared/types/workflows.ts`) — write the entity model exclusively via the `cyboflow_*` MCP tools (`cyboflow_create_task`, `cyboflow_report_finding`, …) — never markdown state files. See `docs/ARCHITECTURE.md` "Data Model" for the authoritative table-by-table breakdown and `docs/CODE-PATTERNS.md` for the chokepoint patterns.
...
> **Codex / OpenAI integration reversed (current source of truth).** Of the list above, the Codex/OpenAI cut did not hold — Codex now ships as a second agent provider (`CodexPtyManager` / `CodexSdkManager` under `main/src/services/panels/codex/`), with per-agent runtime pins available via a workflow's `agentConfigs` overlay. The rest of the out-of-scope list is unaffected.
...
**Workflow self-containment (was "SoloFlow integration").** The first Cyboflow flows were adapted from SoloFlow workflows; the P0 rip-out brought their prompt bodies **into the app source** (`main/src/orchestrator/workflows/*.md`) and severed the runtime dependency on the SoloFlow plugin cache (`~/.claude/plugins/cache/soloflow/...`). The shipped app exposes five user-facing built-in flows — **Launch**, **Planner**, **Sprint**, **Compound**, and **Ship** — plus the internal, picker-hidden `verify-setup` flow, keyed by `CYBOFLOW_WORKFLOW_NAMES` in `shared/types/workflows.ts`; `compound` was rebuilt natively from the preserved prose, and `ship` and `launch` were built natively from scratch. The dropped `prune` flow's prose is preserved under `docs/workflows-future/` (`compound`'s preserved prose also remains there although `compound` has since been rebuilt natively); `soloflow` was the external plugin these flows originated from, not a dropped flow. Agents write the DB via the `cyboflow_*` MCP tools, not markdown state files. The reason Cyboflow exists is because the plugin approach was fundamentally limited in its capability to execute these workflows effectively — Cyboflow now owns the full flow definitions natively.

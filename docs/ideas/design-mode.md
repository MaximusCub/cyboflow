...
**Scope hint:** large (decomposes into epics). **Status:** draft rev 3 (Codex round 1: 8 findings folded; round 2: 5/8 confirmed resolved, 3 reworked + 1 new finding folded), not yet filed to backlog.
...
2. *Sole-writer channel, CSP-enforced.* The captured serialization is **sanitized parent-side** (strip `<script>`, all `on*` handler attributes, `javascript:`/`vbscript:` URLs, `<object>`/`<embed>`/nested frames, SVG script/handler vectors) and re-rendered in a **static comment frame whose CSP is `default-src 'none'; style-src 'unsafe-inline'; img-src data:; script-src 'nonce-<R>'`** where only the app-owned inspector script carries the nonce. Under a nonce-only `script-src`, inline event handlers and `javascript:` URLs cannot execute *even if the sanitizer misses a vector* — the CSP is the enforcement, the sanitizer is defense in depth. The inspector is thus the frame's only possible writer; the parent additionally validates message schema, caps payload size, and rate-limits. Inspector output is treated as UI input (element stacks for anchoring), never as a security decision.
3. *Tests:* payloads using handler attributes (`onclick`, `onerror`), `javascript:` URLs, and SVG handlers must not execute in the comment frame; a prototype that builds DOM at runtime must show that DOM in the freeze.
...
- States: `draft → queued → dispatched → applied | failed`, with a unique **delivery-attempt id** per dispatch.
- `queued → dispatched` commits **when the SDK accepts the turn** (acceptance recorded with the attempt id) — not when delivery is merely attempted.
- `dispatched → applied` requires an **explicit agent acknowledgement** naming the batch id and the resulting prototype artifact revision (via the design-scoped ack tool; the revision turn's prompt carries the batch id and instructs the ack). This correlates feedback to the exact revision that addressed it.
- **Boot recovery is decidable per state:** `queued` → re-dispatch (new attempt id); `dispatched` without ack → the crash window is ambiguous *but attributable*: the sweep re-delivers with the **same batch id** and an idempotency instruction ("if batch N is already addressed in the current prototype, ack without changes") so duplicate application converges instead of double-revising; `applied` is terminal. Nothing silently reverts sent feedback to drafts, and nothing retries into duplicate revisions.
- Tests restart the app before dispatch, after SDK acceptance, during regeneration, and after artifact report but before ack.
...
- Comment mode: the freeze captures live JS-built DOM (test); handler-attribute / `javascript:` URL / SVG-handler payloads do not execute in the comment frame (nonce-CSP tests); the inspector is the sole nonce-carrying script; parent validates schema/size/rate.
- Feedback outbox: both tables accept the new atype; every crash-window restart test (before dispatch, after SDK acceptance, during regeneration, after report before ack) recovers without lost feedback or duplicate revisions; `applied` always correlates a batch id to a prototype artifact revision.

...
|---|---|
| `fleet_*` tools (`fleet_spawn`, `fleet_proposal_*`, `fleet_verification_*`) | **In-session only.** Registered via `pi.registerTool({ name, parameters, execute })` into the OMP session's ExtensionAPI. The only `registerTool` consumers in the repo are test harnesses (`integration-gates.test.ts:280`, `proposals/index.test.ts:127`). There is **no** MCP server, no `@modelcontextprotocol`/fastmcp integration, no HTTP tool endpoint, no Unix-socket RPC exposing the Fleet controller. |

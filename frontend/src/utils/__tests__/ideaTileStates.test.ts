...
describe('deriveIdeaTileStates — shape', () => {
  it('returns exactly the four tiles in clarify → design → planner → ship order', () => {
    const states = deriveIdeaTileStates(ledger('incomplete'), IDLE);
...
describe('deriveIdeaTileStates — recommended next', () => {
  ...
  it('recommends Ship when everything is settled', () => {
    const states = deriveIdeaTileStates(ledger('complete'), IDLE);
  ...
  it('treats a MISSING component as unsettled (never assumes done)', () => {
    ...
    const states = deriveIdeaTileStates([entry('idea-spec', 'complete')], IDLE);
    expect(tile(states, 'design').recommended).toBe(true);
  });
});

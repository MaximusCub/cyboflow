...
export interface OmpSpawnRequest {
  ...
  cwd?: string;
  timeoutMs?: number;
  /** Worker scope (producer WorkerScope); carried verbatim, never widened locally. */
  scope?: unknown;
}
...
/**
 * Result of any OMP command. Every result carries an operationId so a caller
 * can correlate the audit trail and retry idempotently. `blocked` and `shadow`
 * are distinct from a plain failure: a BLOCKED candidate is not a PASS, and

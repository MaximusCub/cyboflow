-- Migration 112: Add nullable origin_idea_id to sessions — sidebar nesting
-- lineage for the idea-session home concept.
--
-- origin_idea_id points at the idea whose LAUNCH minted this session (planner/
-- ship/design launch, as today). Unlike home_idea_id (111), many sessions may
...
-- as 111 and design_idea_id (082): sessions is a legacy table, integrity is
-- chokepoint-enforced. NULL means "no recorded launch origin" (byte-identical
-- behavior to before this column existed).
--
-- Single-statement file for the same replay-hazard reason as 111 (see its

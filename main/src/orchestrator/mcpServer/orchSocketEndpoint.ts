 ...
 * endpoint becomes a NAMED PIPE — Node's `net` module treats a `\\.\pipe\…`
 * path transparently on Windows, which keeps OrchSocketServer
 * (listen/probe/close) and every client (`net.createConnection`) source-
 * unchanged. The pipe namespace is machine-global, so the name folds in TWO
 * discriminators:
 *
 ...
 *      enforces single-instance PER KIND, each with its own `~/.cyboflow*`
 *      data dir), and the per-kind socket path is the per-kind discriminator:
 *      it already differs per data dir. Without the hash both kinds computed
 *      the SAME pipe name, so the second kind's MCP subprocesses connected to
 *      the FIRST kind's orch server — writing through the wrong database.
 *
 * Security note: the POSIX path relied on 0700/0600 modes (OrchSocketServer's
 * tightenMode). Named pipes carry no such modes — the per-user, per-data-dir
 * name plus the run-scoped bearer tokens (orchAuthToken.ts) are the Windows
 * access control. tightenMode degrades to a warn-only no-op there, which is
 * by design.
 *
...
export function orchSocketEndpoint(
  ...
  // handful of concurrently-runnable app kinds apart, without approaching the
  // pipe-name length budget. Not a secret and needn't be cryptographic — it
  // only has to be stable per path and differ across paths.
